...
Android Tool Suite（ATS）定位为“单一宿主中的个人工具平台”：用户只安装一个主应用，在其中安装、运行、备份和更新多个工具；普通工具采用 Web UI 和稳定 Capability API，Android 或 Shizuku 等平台能力由受信 Native Provider 提供。
...
以 [plugin-runtime-v2-refactor-plan.md](plugin-runtime-v2-refactor-plan.md) 为唯一实施计划。核心是 Web Tool、可选 WASM Worker、Capability Router、Native Provider、Storage/Secret/Scheduler Service 以及 API1 到 v2 的正式迁移和回滚。
...
| Host + 通用 Sandbox 双 APK、远程 Compose UI | 放弃实现 | 原型分支只保留研究证据；v2 改用 Web UI 和可替换 Backend |
