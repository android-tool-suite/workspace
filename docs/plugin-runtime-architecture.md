...
2.0.0 进一步移除宿主和 Debug 的旧插件存储、安装回滚、导出、启停和依赖管理分支。旧安装记录不再影响同 ID format v3 插件的导入与状态，旧文件和业务数据不自动删除。历史宿主迁移包只恢复兼容设置，不恢复 API1 插件或沿用旧内置 Shizuku 的启用状态；Dataset 备份适配器与旧数据归档读取兼容继续保留。
