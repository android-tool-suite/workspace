...
- 普通 format v3 Tool、全信任 `trusted-provider` 与 API1 的安全边界必须分别表述；
...
- [ ] 在当前锁定提交上重新执行完整 `tools/build-all.ps1`，刷新集中产物、校验和与 `build-manifest.json`，确保清单不再引用整理前提交或 `uncommitted` 状态。
- [ ] 为 Host/WebView renderer 补齐冷启动、旋转、前后台恢复、Provider 重连和错误外壳回归用例。
- [ ] 为 Scheduler 补齐手动、周期、约束、Provider 事件、超时、重试、并发租约、撤权停调度和重启恢复测试。
- [ ] 增加一个独立于无障碍自动授权的后台任务示例，覆盖任务历史、状态展示和无敏感载荷日志。
- [ ] 在模拟器完成自动化后，通过 ADB 在实体设备安装当前集中产物，复核主页组件、权限撤销、Shizuku 恢复和无障碍自动启用。

完成条件：当前锁定组合具有可追溯构建清单；renderer、Capability、Provider 和 Scheduler 的关键生命周期具有可重复证据。

### P0.1 迁移 Phigros Data Studio

- [ ] 冻结现有 API1 功能、Dataset、SessionToken、缓存、历史记录和生成图片的迁移 fixture。
- [ ] 定义 format v3 清单、声明式 UI 入口、主页组件、后台任务及所需 Capability；不暴露 Android/Kotlin/Compose 类型。
- [ ] 将简单页面迁移到 Host renderer；只有确实需要复杂交互的页面使用 WebView renderer，并复用现行设计 token、状态与文案规范。
- [ ] 将网络、文件导入、凭据、数据集和后台行为接入稳定 Capability，不允许普通 Tool 直接访问宿主或 Shizuku 实现。
- [ ] 将 SessionToken 写入 SecretStore，将账号、缓存和历史数据迁移到 Dataset generation；写入使用 staging、校验和原子切换。
- [ ] 实现从已发布 API1 数据的单向导入，失败时保持旧数据和当前 generation 不变。
- [ ] 通过 RKS、存档解析、历史合并、缓存、图片生成及错误路径的 JVM/契约测试。
- [ ] 完成旧数据导出、空环境恢复、业务结果校验、降级演练和实体设备验收。
- [ ] 发布迁移版 Debug，确认更新、回滚和旧版共存策略后再发布正式版。

完成条件：用户不再依赖 Phigros API1 代码即可获得业务等价功能，旧数据可验证迁移并可降级恢复。

### P0.2 迁移跃迁与祈愿分析

- [ ] 冻结原神、星穹铁道账号、记录、完成状态、设置和 UIGF 导入导出的迁移 fixture。
- [ ] 定义 format v3 清单、声明式 UI、主页组件、任务和网络／文件／存储 Capability。
- [ ] 将页面迁移到统一声明式入口，保持原神与星穹铁道的筛选、统计、导入、导出和错误状态业务等价。
- [ ] 将数据获取与 Shizuku 具体实现解耦；只有确需系统身份的高层能力才依赖 Provider，普通逻辑使用受限 Worker。
- [ ] 将账号、记录、池状态和设置迁移到 Dataset generation；敏感数据进入 SecretStore 或受保护 Dataset。
- [ ] 实现从 API1 数据的单向导入、合并冲突处理、失败回滚和单游戏删除隔离。
- [ ] 通过 UIGF、记录合并、分页、去重、双游戏隔离、旧 fixture 和权限拒绝测试。
- [ ] 完成旧数据导出、空环境恢复、业务结果校验、降级演练和实体设备验收。
- [ ] 发布迁移版 Debug，确认更新与回滚后再发布正式版。

完成条件：两个游戏的数据与主要功能均不再依赖 API1，同一迁移或删除操作不会破坏另一游戏的数据。

### P0.3 正式发布与 API1 退出

- [ ] 所有剩余插件完成迁移后，集中执行旧数据导出、空环境恢复、业务校验、升级与降级演练；全部通过后即可退出 API1 与 Bridge。
- [ ] API1 退出只以迁移完成和测试结果为条件，不再附加稳定版本数量或日历时间要求。
- [ ] 停止发布新的 API1 插件版本，并从模板、示例和开发文档移除 API1 创建入口。
- [ ] 删除 Bridge 写入路径、旧 AAR Tool API、旧插件装载和已确认无用的兼容代码。
- [ ] 保留 `.atsbackup` v2/v3 与必要旧清单的最小只读解析能力，确保历史归档仍可识别和迁移。
- [ ] 删除 API1 后重新执行全量构建、升级、历史归档读取和实体设备回归测试。
- [ ] 发布包含 Host、SDK、Shizuku、无障碍及两个迁移插件，且不再包含 API1 运行路径的正式稳定组合。
- [ ] 更新兼容矩阵、CHANGELOG、迁移说明、发布索引和回滚手册。
